...
use lsp_proxy::text::{find_symbol_location, get_word_at_position, rank_symbol_locations};
use serde_json::json;
...
fn test_proxy_definition_fallback_flow() {
    ...
    // 4. Rust-analyzer returns candidate symbols
    ...
    // 5. Proxy transforms RA response into Zed definition response with ranked locations
    ...
    // Best match (Struct Harness) ranked first
    assert_eq!(locations[0]["uri"], "file:///path/to/harness.rs");
    assert_eq!(locations[0]["range"]["start"]["line"], 10);

    // Prefix match (HarnessHelper) ranked second
    assert_eq!(locations[1]["uri"], "file:///path/to/helper.rs");
}
...
fn test_find_and_rank_symbol_location_heuristics() {
    ...
    // Fallback to first if name not matched
    let loc_fallback = find_symbol_location(&symbols, "unknown").unwrap();
    assert_eq!(loc_fallback["uri"], "file:///foo_fn.rs");

    // Ranking returns both locations ordered by relevance
    let ranked = rank_symbol_locations(&symbols, "Target");
    assert_eq!(ranked.len(), 2);
    assert_eq!(ranked[0]["uri"], "file:///target_struct.rs");
    assert_eq!(ranked[1]["uri"], "file:///foo_fn.rs");
}
