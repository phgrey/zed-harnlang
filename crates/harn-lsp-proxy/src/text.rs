...
/// Filters, ranks, and deduplicates candidate symbol locations for a queried word.
///
/// Only EXACT symbol names (or exact case-insensitive matches for types) are accepted.
/// Fuzzy, substring, or prefix matches from rust-analyzer are discarded to prevent
/// cluttering the editor with unrelated definitions.
///
/// Priority ranking:
/// 1. Exact case-sensitive match on Types (Struct, Enum, Class, Interface: 22, 13, 5, 11) -> 100
/// 2. Exact case-sensitive match on Functions / Methods (12, 6) -> 90
/// 3. Exact case-sensitive match on other symbols -> 80
/// 4. Case-insensitive exact match on Types (e.g. `harness` -> `Harness`) -> 70
/// 5. Case-insensitive exact match on other symbols -> 60
pub fn rank_symbol_locations(results: &[Value], word: &str) -> Vec<Value> {
    ...
    for sym in results {
        ...
        let score = if name == word {
            ...
            // Discard all non-exact / fuzzy matches
            continue;
        };
...
mod tests {
    ...
    #[test]
    fn test_rank_symbol_locations_filters_unrelated_and_dedups() {
        let results = vec![
            json!({
                "name": "HarnessHelper", // Substring/prefix - MUST be dropped
                "kind": 12,
                "location": {"uri": "file:///src/helper.rs"}
            }),
            json!({
                "name": "test_harness", // Substring - MUST be dropped
                "kind": 12,
                "location": {"uri": "file:///src/test.rs"}
            }),
            json!({
                "name": "Harness",
        ...
        // Only the exact matches kept and deduplicated (helper & test dropped)
        assert_eq!(ranked.len(), 2);
        // Struct first (score 100)
        assert_eq!(ranked[0]["uri"], "file:///src/struct.rs");
        // Function second (score 90)
        assert_eq!(ranked[1]["uri"], "file:///src/func.rs");
    }

    #[test]
    fn test_rank_symbol_locations_case_insensitive_type() {
        let results = vec![
            json!({
                "name": "Harness", // Struct in Rust, queried as lowercase "harness"
                "kind": 22,
                "location": {"uri": "file:///src/struct.rs"}
            }),
            json!({
                "name": "harness_util", // Discarded
                "kind": 12,
                "location": {"uri": "file:///src/util.rs"}
            }),
        ];
    ...
    fn test_find_symbol_empty_or_no_location() {
        ...
        // Substring non-match returns None
        let results_unmatched =
            vec![json!({"name": "HarnessContext", "location": {"uri": "file:///x"}})];
        assert_eq!(find_symbol_location(&results_unmatched, "Harness"), None);
    }
}
