...
use harn_lsp_proxy::server::{Server, tx_for_std};
use harn_lsp_proxy::text::word_at_cursor;
use harn_lsp_proxy::transport::read_message;
use log::info;
        ...
        while let Some(msg) = read_message(&mut stdout).await {
            if let Some(ref id) = msg.id_str() {
                ...
                if let Some(params) = params
                    && msg.is_empty_result()
                {
                    if let Some(word) = word_at_cursor(&params) {
                        info!(
